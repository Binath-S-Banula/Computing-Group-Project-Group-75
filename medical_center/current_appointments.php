...
// Fetch upcoming medical appointments (today onwards)
try {
    $today = date('Y-m-d');
    $stmt = $pdo->prepare("
        SELECT 
        ...
        WHERE ma.appointment_date >= ?
        AND ma.status IN ('approved', 'pending')
        ORDER BY 
            CASE 
                WHEN ma.status = 'approved' THEN 1
                WHEN ma.status = 'pending' THEN 2
            END,
            ma.appointment_date ASC
    ");
    $stmt->execute([$today]);
...
<div class="dashboard-title d-flex justify-content-between align-items-center mb-4">
    <h2 class="card-title">
        <i class="fas fa-calendar-check"></i> Active Appointments
    </h2>
...
<?php if (!empty($appointments)): ?>
    <div class="table-responsive">
        ...
        <div class="btn-div">
            <a href="view_appointments.php" class="view-back">
